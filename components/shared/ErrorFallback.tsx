...
export function ErrorFallback({ featureName, message, error }: IErrorFallback) {
  ...
  return (
    ...
    >
      ...
      <div className="text-description flex flex-col gap-1 text-center text-sm">
        <p>{message}</p>
      </div>
      ...
      {process.env.NODE_ENV === 'development' && (
        <details className="mt-4 text-xs text-gray-400">
          ...
          <pre className="mt-2 h-full w-full overflow-auto rounded bg-gray-100 p-2 text-left">
            {error.message}
          </pre>
        </details>
      )}
