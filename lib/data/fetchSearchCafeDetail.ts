...
export async function fetchSearchCafeDetail(cafeId: string): Promise<ISearchCafeDetail | null> {
  ...
  try {
    const fetchUrl = process.env.NEXT_PUBLIC_BASE_URL === 'http://localhost:3000' ? '/api/cafe-detail/local' : '/api/cafe-detail';
    ...
    if (error instanceof Error) {
      if (error.name === 'AbortError') throw new Error('Fallback API 요청 시간 초과');
      throw error;
    }

    throw new Error('API 요청 중 알 수 없는 오류 발생');
  }
}