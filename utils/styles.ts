...
export const getSidebarStyle = (
  ...
  return `${isDarkTheme ? 'bg-darkbg text-white' : 'bg-white'} ${isSubSidebarOpen ? 'hidden sm:block' : ''} z-10 relative w-screen h-screen max-w-[22rem] rounded-none shadow-xl shadow-mainShadow flex flex-col justify-between overflow-y-scroll overflow-x-hidden`;
};
